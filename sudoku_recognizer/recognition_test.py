...
mean_thresh_inv = cv2.adaptiveThreshold(
    grayscaled, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY_INV, 21, 17)
mean_thresh_norm = cv2.adaptiveThreshold(
    grayscaled, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 21, 17)
print("preprocessing done")

# find the main square with findContours
img_contours, contours, hierarchy = cv2.findContours(
    mean_thresh_inv, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
max_size_square = 0
...
cv2.imwrite("unscaled.jpg", resized_mean_thresh_norm)
height_2, width_2 = resized_mean_thresh_norm.shape[0:2]
...
for part in parts:
    ...
    im = im.resize((110, 110), Image.BICUBIC)
    area = (10, 10, 100, 100)
    im = im.crop(area)
